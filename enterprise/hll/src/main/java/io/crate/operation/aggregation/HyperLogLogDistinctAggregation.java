...
public class HyperLogLogDistinctAggregation extends AggregationFunction<HyperLogLogDistinctAggregation.HllState, Long> {
    ...
    public static class HllState implements Comparable<HllState>, Writeable {
        ...
        public int compareTo(HllState o) {
            return Long.compare(hyperLogLogPlusPlus.cardinality(0), o.hyperLogLogPlusPlus.cardinality(0));
        }
    ...
    abstract static class Murmur3Hash {
        ...
        static Murmur3Hash getForType(DataType dataType, Version indexVersionCreated) {
            switch (dataType.id()) {
                case DoubleType.ID:
                case FloatType.ID:
                    return Murmur3HashDouble.INSTANCE;
                case LongType.ID:
                case IntegerType.ID:
                case ShortType.ID:
                case ByteType.ID:
                case TimestampType.ID:
                    return Murmur3HashLong.INSTANCE;
                case StringType.ID:
                case BooleanType.ID:
                    return Murmur3HashBytes.INSTANCE;
                case IpType.ID:
        ...
        private static class Murmur3HashLong extends Murmur3Hash {

            private static final Murmur3HashLong INSTANCE = new Murmur3HashLong();
        ...
        private static class Murmur3HashDouble extends Murmur3Hash {

            private static final Murmur3HashDouble INSTANCE = new Murmur3HashDouble();

            @Override
            long hash(Object val) {
                return BitMixer.mix64(Double.doubleToLongBits(DataTypes.DOUBLE.value(val)));
            }
        }

        private static class Murmur3HashBytes extends Murmur3Hash {

            private static final Murmur3HashBytes INSTANCE = new Murmur3HashBytes();
